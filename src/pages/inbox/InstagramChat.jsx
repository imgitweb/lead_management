...
import { fetchInstaChats, fetchInstaMessages, sendInstaReply } from "../../services/instagramApi";
...
const InstagramChat = () => {
  ...
  // 1. Component mount hone par chats load karein
  useEffect(() => {
    loadChats();
  }, []);

  // 2. Naya message aane par auto-scroll karein
  useEffect(() => {
  ...
  const loadChats = async () => {
    ...
    try {
      const data = await fetchInstaChats();
      setChats(data);
    } catch (error) {
      toast.error("Error", "Failed to load Instagram chats");
    } finally {
  ...
  const handleSelectChat = async (chat) => {
    ...
    try {
      const chatHistory = await fetchInstaMessages(chat._id);
  ...
  const handleSendMessage = async (e) => {
    ...
    setSending(true);
    try {
      const response = await sendInstaReply(
        selectedChat._id,
        replyText,
      ...
      // Sidebar me last message update
      setChats((prevChats) =>
        prevChats.map((c) =>
  ...
  return (
    ...
    >
      ...
      <div className={`flex-shrink-0 mb-2 md:mb-4 ${selectedChat ? 'hidden md:block' : 'block'}`}>
        <Breadcrumb items={[{ label: 'Dashboard', path: '/' }, { label: 'Instagram' }]} />
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <FaInstagram size={22} style={{ color: theme.primary }} /> IG Messages
            </h2>
            <Button 
              variant="outline" 
              onClick={loadChats} 
        ...
        >
          {selectedChat ? (
            ...
            >
              ...
              >
                <FaInstagram size={40} style={{ color: theme.primary }} />
              </div>
